...
public class CmdLine {
    ...
    public static String CWC_TAIL_LENGTH = "13";
    ...
    public static Map<String, Double> parseBlackboxInput(String filePath, StrategyEnum strategy) throws IOException {
        try(BufferedReader br = Files.newBufferedReader(Paths.get(filePath))) {
            ...
            if(strategy.equals(StrategyEnum.VB)) {
                ...
                if (params.length != 5) {
                    throw new RuntimeException("Invalid format (TRADING_WINDOW_SIZE(int) CWC_TAIL_LENGTH(double<1) STOP_LOSS_PCT(double) TS_TRIGGER_PCT(double<1) TS_PCT(double<1)");
                }
                result.put(CmdLine.TRADING_WINDOW_SIZE_IN_MIN, Double.valueOf(params[0]));
                result.put(CmdLine.CWC_TAIL_LENGTH, Double.valueOf(params[1]));
                result.put(CmdLine.STOP_LOSS_PCT, Double.valueOf(params[2]));
                result.put(CmdLine.TS_TRIGGER_PCT, Double.valueOf(params[3]));
                result.put(CmdLine.TS_PCT, Double.valueOf(params[4]));
